...
namespace AOI
{
    ...
    public class AoiNodeLinkedList : LinkedList<AoiNode>
    {
        ...
        public void Insert(AoiNode node)
        {
            ...
            else if (_linkedListType == AoiNodeLinkedListType.YLink)
            {
                InsertY(node);
            }
            else 
        ...
        private void InsertZ(AoiNode node)
        {
            if (First == null)
            {
                node.Link.ZNode = AddFirst(AoiPool.Instance.Fetch<LinkedListNode<AoiNode>>(node).Value);
            }
            else
            {
                var slowCursor = First;
                ...
                if (Last.Value.Position.Z > node.Position.Z)
                {
                    for (var i = 0; i < _skipCount; i++)
                    {
                        // 移动快指针
                        ...
                        if (fastCursor.Value.Position.Z <= node.Position.Z)
                        {
                            slowCursor = fastCursor;
                        ...
                        while (slowCursor != null)
                        {
                            if (slowCursor.Value.Position.Z >= node.Position.Z)
                            {
                                node.Link.ZNode = AoiPool.Instance.Fetch<LinkedListNode<AoiNode>>(node);

                                AddBefore(slowCursor,  node.Link.ZNode);
                ...
                if (node.Link.ZNode == null)
                {
                    node.Link.ZNode = AddLast(AoiPool.Instance.Fetch<LinkedListNode<AoiNode>>(node).Value);
                }
            }
        }
        ...
        private LinkedListNode<AoiNode> FastCursor(int skip, AoiNode currentNode)
        {
            ...
            switch (_linkedListType)
            {
                ...
                case AoiNodeLinkedListType.ZLink:
                {
                    for (var i = 1; i <= skip; i++)
                    {
                        if (skipLink.Link.ZNode.Next == null) break;

                        skipLink = skipLink.Link.ZNode.Next.Value;
                    }
                
                    return skipLink.Link.ZNode;
                }
                default:
                    return null;
            }
